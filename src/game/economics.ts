...
import { PLANTS, LCOE_LIFETIME_DAYS, TARIFF } from '../config/components';
...
export function genEconomics(tariff: number = TARIFF): GenEconomics[] {
  return Object.values(PLANTS).map((spec) => {
    const energyPerDay = spec.capacity * spec.cf * 24; // MWh/day
    const fuelPerDay = energyPerDay * spec.marginalCost;
    ...
    return {
      ...
      capex: spec.capex, fuel: spec.marginalCost, omPerDay: spec.omPerDay, cf: spec.cf,
