...
import { FleetPanel } from './ui/fleet';
...
import { Tutorial, TUTORIAL_STEPS, FINANCE_TUTORIAL_STEPS, MARKET_TUTORIAL_STEPS, DERIVATIVES_TUTORIAL_STEPS, type TutorialStep } from './game/tutorial';
import { Advisor } from './game/advisor';
import { NewsSystem } from './game/news';
import { loadMeta, levelProgress, recordRunResult, consumeRpBonus } from './game/meta';
import { MODES, modeById, modeApplicable, type ModeId } from './game/modes';
...
import type { Bus, PlantType } from './sim/types';
import {
...
const TOOL_ORDER: ToolId[] = ['inspect', 'line', 'substation', 'upgrade', 'coal', 'gas', 'wind', 'solar', 'nuclear', 'hydro', 'biomass', 'battery', 'pumped', 'hydrogen', 'datacenter', 'transport', 'petrochem', 'mining', 'maintenance', 'ccs', 'capacitor', 'backup', 'contract', 'bulldoze'];

// 教程关 → 步骤表（新手 + 三个进阶 mini 教程）
const TUTORIAL_BY_SCENARIO: Record<string, TutorialStep[]> = {
  tutorial: TUTORIAL_STEPS,
  tutFinance: FINANCE_TUTORIAL_STEPS,
  tutMarket: MARKET_TUTORIAL_STEPS,
  tutDerivatives: DERIVATIVES_TUTORIAL_STEPS,
};
...
let lastStartups = 0; // 机组启动计数（启动音效边沿触发）
let wasRestoring = false; // 上一帧是否处于停电恢复中（恢复完成音效）
...
let currentModeName: string | undefined; // 本局变体模式名（战绩标注用）
let resultRecorded = false; // 本局是否已计入元进度（防重复）

// 变体模式选择（持久化）
const MODE_KEY = 'powerworld.mode';
let selectedMode: ModeId = 'standard';
try {
  const saved = localStorage.getItem(MODE_KEY);
  if (saved && MODES.some((m) => m.id === saved)) selectedMode = saved as ModeId;
} catch { /* 忽略 */ }

// 撤销栈：记录最近的建造动作（仅在建工程可撤——全额退款；已投运走拆除流程）
const undoStack: { kind: 'bus' | 'line'; id: number }[] = [];

/** 通关时的策略签名：电源组合一句话（分享文本用） */
function strategySig(): string {
  const names: Record<PlantType, string> = { coal: '煤', gas: '气', wind: '风', solar: '光', nuclear: '核', hydro: '水', biomass: '生' };
  const count = new Map<PlantType, number>();
  for (const g of sim.grid.gens.values()) count.set(g.type, (count.get(g.type) ?? 0) + 1);
  const parts = [...count.entries()].map(([t, n]) => `${names[t]}${n}`);
  if (sim.grid.batteries.size) parts.push(`储${sim.grid.batteries.size}`);
  return parts.join('·');
}

/** 通关亮点徽章 */
function badgesOf(): string[] {
  const b: string[] = [];
  if (sim.outageEnergyTotal < 1) b.push('⚡零失负荷');
  if (sim.renewableShare >= 0.8) b.push('🌱全清洁');
  if (sim.debt <= 0) b.push('🏦零负债');
  if (sim.n1Secure) b.push('🛡N-1');
  return b;
}
...
function newGame(scenario: Scenario): void {
  ...
  scenario.setup(sim);
  // 变体模式：叠加在关卡之上（教程/沙盒不叠）
  currentModeName = undefined;
  const mode = modeById(selectedMode);
  const metaLevel = levelProgress(loadMeta().xp).level;
  if (mode.id !== 'standard' && mode.unlockLevel <= metaLevel && modeApplicable(scenario.id) && !sim.sandbox) {
    mode.apply(sim);
    currentModeName = mode.name;
    sim.log('warn', `🎮 变体模式【${mode.name}】生效：${mode.desc}`);
  }
  // 失败补偿：上局失利 → 本局研发点 +10%
  const rpMult = consumeRpBonus();
  if (rpMult > 1) {
    sim.rpRateMult *= rpMult;
    sim.log('good', `🎁 失利补偿：本局研发点积累 +${Math.round((rpMult - 1) * 100)}%`);
  }
  ...
  news.reset();
  enterGame();
  const steps = TUTORIAL_BY_SCENARIO[scenario.id];
  if (steps) tutorial.start(steps);
...
function continueGame(slot: SlotId = 'quick'): void {
  ...
  lastAutosaveDay = sim.day;
  currentModeName = undefined; // 模式效果已固化进存档，标签不再追溯
  resultRecorded = sim.gameOver; // 已结束的存档不再重复计入元进度
  undoStack.length = 0;
  news.reset();
  enterGame();
  tutorial.stop();
...
function enterGame(): void {
  ...
  lastStartups = sim.startupsTotal;
  wasRestoring = false;
  fleetPanel.hide();
  hud.setBuildTip(null);
  hud.setSpeed(0);
  menu.hide();
  menuOpen = false;
  gameActive = true;
...
function openMenu(): void {
  ...
  hud.setSpeed(0);
  const meta = loadMeta();
  const lp = levelProgress(meta.xp);
  menu.show({
    scenarios: SCENARIOS,
    saves: listSaves(),
    bests: allBests(),
    meta: { level: lp.level, frac: lp.frac, xp: meta.xp, wins: meta.wins, losses: meta.losses, rpBonusArmed: meta.pendingRpBonus > 0 },
    modes: MODES.map((m) => ({
      id: m.id, name: m.name, icon: m.icon, desc: m.desc,
      locked: lp.level < m.unlockLevel, unlockLevel: m.unlockLevel, selected: m.id === selectedMode,
    })),
    onSelectMode: (id) => {
      selectedMode = id as ModeId;
      try { localStorage.setItem(MODE_KEY, selectedMode); } catch { /* 忽略 */ }
      openMenu(); // 重渲染选中态
    },
...
function openFleet(): void {
  ...
  fleetPanel.show({
    sim,
    onMaintain: (busId) => {
      if (sim.scheduleMaintenance(busId)) { sound.build(); openFleet(); }
      else { sound.error(); flashHint('无法检修（已离线/在建/资金不足）'); }
    },
    onClose: () => { fleetPanel.hide(); panelOpen = false; },
  });
}

/** 切换线路着色模式：负载 → 电压 → 拥堵 */
function cycleViewMode(): void {
  const order: (typeof renderer.lineColorMode)[] = ['load', 'voltage', 'congestion'];
  const names = { load: '负载率（绿→红）', voltage: '电压（欠压发红）', congestion: '拥堵（>70% 才着色）' } as const;
  renderer.lineColorMode = order[(order.indexOf(renderer.lineColorMode) + 1) % order.length];
  flashHint(`🗺 线路着色：${names[renderer.lineColorMode]}`);
  sound.click();
}

/** 撤销最近一次建造（仅在建工程：全额退款；已投运的请走拆除流程） */
function undoLastBuild(): void {
  while (undoStack.length) {
    const item = undoStack.pop()!;
    if (item.kind === 'bus') {
      const bus = sim.grid.buses.get(item.id);
      if (!bus) continue; // 已被拆除
      const refund = sim.cancelRefund(bus.id);
      if (refund == null) { flashHint('最近的建造已投运——撤销只对在建工程有效（已投运请用拆除）'); return; }
      sim.grid.removeBus(bus.id);
      if (refund > 0) sim.refund(refund);
      invalidateN1();
      sound.build();
      sim.log('info', `↩ 撤销建造 ${bus.name}，全额退款 ¥${refund.toLocaleString('en-US')}`);
      return;
    } else {
      const ln = sim.grid.lines.get(item.id);
      if (!ln) continue;
      const refund = sim.lineCancelRefund(ln);
      if (refund == null) { flashHint('最近的线路已投运——撤销只对在建工程有效'); return; }
      sim.grid.removeLine(ln.id);
      if (refund > 0) sim.refund(refund);
      invalidateN1();
      sound.build();
      sim.log('info', `↩ 撤销架线，全额退款 ¥${refund.toLocaleString('en-US')}`);
      return;
    }
  }
  flashHint('没有可撤销的建造（Ctrl+Z 只撤在建工程）');
}
...
function handleClick(clientX: number, clientY: number): void {
  ...
  switch (tool) {
        ...
        if (bus && bus.id !== pendingFrom.id) {
          ...
          if (!chk.ok) {
            ...
            if (sim.spend(cost)) {
              ...
              undoStack.push({ kind: 'line', id: ln.id });
              invalidateN1();
              sound.build();
    ...
    case 'upgrade': {
      // 优先变电站（换大主变），其次线路（导线增容）
      if (bus && bus.kind === 'substation') {
        if (sim.upgradeTransformer(bus.id)) sound.build();
        else { flashHint(`无法增容（在建/资金不足 ¥${sim.transformerUpgradeCost().toLocaleString('en-US')}）`); sound.error(); }
        return;
      }
      const upLn = renderer.nearestLine(tile.x, tile.y);
      if (upLn) {
        if (sim.upgradeLineCapacity(upLn.id)) sound.build();
        else { flashHint(`无法增容（在建/资金不足 ¥${sim.lineUpgradeCost(upLn).toLocaleString('en-US')}）`); sound.error(); }
        return;
      }
      flashHint('点击一条线路或一座变电站进行增容');
      return;
    }
    case 'substation': {
      const p = snap(tile);
      ...
      if (sim.spend(cost)) {
        ...
        undoStack.push({ kind: 'bus', id: sub.id });
        invalidateN1();
        sound.build();
    ...
    case 'hydrogen': {
      ...
      if (sim.spend(cost)) {
        ...
        undoStack.push({ kind: 'bus', id: bbus.id });
        invalidateN1();
        sound.build();
    ...
    case 'mining': {
      ...
      if (sim.spend(acqCost)) {
        ...
        undoStack.push({ kind: 'bus', id: lbus.id });
    ...
    default: {
      ...
      if (!type) return;
      if (sim.bannedPlants.has(type)) { flashHint(`🚫 本局禁建${PLANTS[type].label}（关卡规则）`); sound.error(); return; }
      const p = snap(tile);
      if (renderer.nearestBus(p.x, p.y, 0.7)) { flashHint('此处已有设备'); sound.error(); return; }
      ...
      if (sim.spend(cost)) {
        ...
        undoStack.push({ kind: 'bus', id: pbus.id });
        invalidateN1();
        sound.build();
...
/** 建造预览：随光标更新合法性着色 + 造价/资源浮签（建造类游戏的基础反馈） */
function updateBuildPreview(tile: { x: number; y: number }, hover: Bus | null, clientX: number, clientY: number): void {
  renderer.cursorOk = null;
  renderer.pendingLineOk = null;
  if (menuOpen || panelOpen || sim.gameOver) { hud.setBuildTip(null); return; }
  const tool = hud.currentTool;
  const money = (cost: number) => sim.money >= cost;
  const row = (k: string, v: string) => `<div>${k} <b>${v}</b></div>`;

  // 拉线中：对悬停目标做合法性预检 + 实时造价
  if (tool === 'line' && pendingFrom) {
    if (hover && hover.id !== pendingFrom.id) {
      const chk = sim.grid.canConnect(pendingFrom.id, hover.id);
      if (!chk.ok) {
        renderer.pendingLineOk = false;
        hud.setBuildTip(`<span class="bt-bad">✗ ${chk.reason ?? '无法连接'}</span>`, clientX, clientY);
      } else {
        const cost = sim.grid.lineCost(pendingFrom.id, hover.id);
        const ok = money(cost);
        renderer.pendingLineOk = ok;
        hud.setBuildTip(
          row(VOLTAGE[chk.voltage!].label, '') + row('造价', `¥${cost.toLocaleString('en-US')}`) + (ok ? '' : `<span class="bt-bad">资金不足</span>`),
          clientX, clientY,
        );
      }
    } else {
  ...
  // 增容工具：悬停对象显示费用
  if (tool === 'upgrade') {
    if (hover && hover.kind === 'substation') {
      const cost = sim.transformerUpgradeCost();
      renderer.cursorOk = money(cost);
      hud.setBuildTip(row('换大主变 +45MW', `¥${cost.toLocaleString('en-US')}`), clientX, clientY);
    } else {
      const ln = renderer.nearestLine(tile.x, tile.y);
      if (ln) {
        const cost = sim.lineUpgradeCost(ln);
        hud.setBuildTip(row(`线路增容 +${Math.round(VOLTAGE[ln.voltage].defaultCapacity * 0.5)}MW`, `¥${cost.toLocaleString('en-US')}`), clientX, clientY);
      } else hud.setBuildTip(null);
    }
    return;
  }
  // 落地建造类：占位/禁令/造价/资源评分
  const plantType = PLANT_TOOLS[tool];
  const isStorage = tool === 'battery' || tool === 'pumped' || tool === 'hydrogen';
  const isKey = tool in KEY_ACCOUNTS;
  if (plantType || isStorage || isKey || tool === 'substation') {
    const p = snap(tile);
    const occupied = renderer.nearestBus(p.x, p.y, 0.7) != null;
    const banned = plantType != null && sim.bannedPlants.has(plantType);
    let base = 0;
    if (plantType) base = PLANTS[plantType].capex;
    else if (isStorage) base = STORAGE[tool as 'battery' | 'pumped' | 'hydrogen'].capex;
    else if (tool === 'substation') base = SUBSTATION_CAPEX;
    const cost = isKey ? sim.keyAccountAcquireCost(KEY_ACCOUNTS[tool].profile) : sim.buildCostAt(p.x, p.y, base);
    const rejected = isKey && cost < 0;
    const ok = !occupied && !banned && !rejected && money(Math.max(cost, 0));
    renderer.cursorOk = ok;
    const lines: string[] = [];
    if (occupied) lines.push('<span class="bt-bad">✗ 此处已有设备</span>');
    else if (banned) lines.push('<span class="bt-bad">🚫 本局禁建该电源</span>');
    else if (rejected) lines.push('<span class="bt-bad">✗ 竞争力不足，大客户拒绝入驻</span>');
    else {
      lines.push(row('造价', `¥${cost.toLocaleString('en-US')}`));
      const tf = base > 0 ? cost / base : 1;
      if (tf > 1.02) lines.push(row('地形抬价', `×${tf.toFixed(2)}`));
      if (plantType === 'wind' || plantType === 'solar' || plantType === 'hydro') {
        const q = sim.grid.terrain.siteQuality(plantType, p.x, p.y);
        const tag = q >= 1.1 ? '<span class="bt-good">✨ 优质场址</span>' : q < 0.9 ? '<span class="bt-bad">⚠ 贫资源区</span>' : '一般';
        lines.push(row('资源', `${(q * 100).toFixed(0)}% ${tag}`));
      }
      if (!money(Math.max(cost, 0))) lines.push('<span class="bt-bad">资金不足</span>');
    }
    hud.setBuildTip(lines.join(''), clientX, clientY);
    return;
  }
  hud.setBuildTip(null);
}
...
function bindInput(): void {
  ...
  window.addEventListener('pointermove', (e) => {
    ...
    updateBuildPreview(tile, hover, e.clientX, e.clientY);
    if (dragging) {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && (e.key === 'z' || e.key === 'Z')) { e.preventDefault(); undoLastBuild(); return; }
    if (e.key === 'h' || e.key === 'H') { renderer.fitView(); return; }
    if (e.key === 'm' || e.key === 'M') { cycleViewMode(); return; }
...
async function start(): Promise<void> {
  ...
  hud.onFleet = openFleet;
  hud.onViewMode = cycleViewMode;
  ...
  renderer.app.ticker.add(() => {
    ...
    if (!menuOpen && !panelOpen) {
      ...
      // 程序化新闻：每日头条 + 季度城市报告（叙事层）
      news.update(sim);
      // 严重事件（跳闸/破产）触发报警音
      if (sim.badEventCount > lastBadEvents) sound.trip();
      lastBadEvents = sim.badEventCount;
      // 机组并网启动音（边沿触发）
      if (sim.startupsTotal > lastStartups) sound.startup();
      lastStartups = sim.startupsTotal;
      // 黑启动恢复完成音：从"恢复中"回到全网通电
      const energized = sim.gridEnergized;
      if (wasRestoring && energized > 0.995) { sound.restore(); hud.toast('🔌 全网恢复供电'); }
      wasRestoring = energized < 0.95;
      // 状态感知 BGM：可靠性告急/停电恢复中 → 音乐变紧张
      const stress = Math.max(
        sim.reliability < sim.goalReliability ? (sim.goalReliability - sim.reliability) * 6 : 0,
        energized < 0.99 ? 0.8 : 0,
      );
      sound.setTension(Math.min(1, stress));
      // 输赢瞬间：音效 + 战绩归档（个人最佳/分享文本）+ 元进度（XP/等级/失利补偿）
      if (sim.gameOver && !wasGameOver) {
        ...
        if (sim.win) {
          ...
          lastScore = {
            ...
            sig: strategySig(), badges: badgesOf(), mode: currentModeName,
          };
        ...
        // 元进度：胜负都计入（每一局都算数）
        if (!resultRecorded && !sim.sandbox) {
          resultRecorded = true;
          const mr = recordRunResult(sim.win, g.grade);
          if (!sim.win) hud.toast(`📈 +${mr.gainedXp} XP · 已武装下局研发点 +10%（失利补偿）`);
          else hud.toast(`📈 +${mr.gainedXp} XP`);
          if (mr.leveledUp) {
            hud.toast(`⭐ 指挥官升级 → Lv.${mr.level}！查看主菜单的变体模式解锁`);
            sound.unlock();
          }
        }
      }
