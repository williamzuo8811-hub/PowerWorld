...
export const PLANTS: Record<PlantType, PlantSpec> = {
  ...
  hydro: {
    type: 'hydro', label: '水电', capacity: 50, pmin: 5, rampRate: 1.8,
    marginalCost: 4, capex: 420_000, buildDays: 6, omPerDay: 350, cf: 0.45, dispatchable: true, co2: 0,
    startupCost: 300, minUpHours: 0.25, minDownHours: 0.25,
    color: 0x2dd4bf, desc: '清洁可调·爬坡快·看来水(夏丰冬枯)·可黑启动·投资高',
  },
  biomass: {
    type: 'biomass', label: '生物质', capacity: 25, pmin: 8, rampRate: 0.3,
    marginalCost: 48, capex: 140_000, buildDays: 2.5, omPerDay: 260, cf: 0.65, dispatchable: true, co2: 0.02,
    startupCost: 4_000, minUpHours: 3, minDownHours: 2,
    color: 0x84cc16, desc: '清洁基荷·可调度·燃料偏贵·近零碳',
  },
};

// —— 水电季节性来水：夏季丰水满发、冬季枯水出力受限（与季节系统联动）——
export const HYDRO_AVAIL_BASE = 0.85; // 春/秋平水期可用系数
export const HYDRO_SUMMER_BOOST = 0.15; // 夏季丰水加成（至满发）
export const HYDRO_WINTER_DROP = 0.45; // 冬季枯水折减
...
export const PLANT_FUEL: Record<PlantType, FuelType | null> = {
  coal: 'coal', gas: 'gas', nuclear: 'uranium', wind: null, solar: null, hydro: null, biomass: null,
};
...
export const IMPORT_MARKUP = 1.25; // 购电相对市价的加价（按现货与均价孰高计——稀缺时进口同样昂贵）
...
/** 对手性格：green=清洁扩张型（扩张快/退役慢）；coal=火电基荷型（受环保督查限产）；peaker=激进调峰型（会打价格战） */
export type CompetitorStyle = 'green' | 'coal' | 'peaker';
export interface CompetitorSpec { name: string; capacity: number; marginalCost: number; style: CompetitorStyle; }
export const REGIONAL_BASE_DEMAND = 520; // 区域市场基准需求 (MW)
export const COMPETITORS_INIT: CompetitorSpec[] = [
  { name: '绿源电力', capacity: 150, marginalCost: 6, style: 'green' }, // 廉价清洁基荷
  { name: '蓝煤集团', capacity: 200, marginalCost: 24, style: 'coal' }, // 廉价火电基荷
  { name: '峰谷能源', capacity: 110, marginalCost: 56, style: 'peaker' }, // 昂贵调峰
];
// 性格化行为参数
export const COMP_GREEN_EXPAND_MULT = 1.5; // 清洁型扩张提速
export const COMP_GREEN_RETIRE_MULT = 0.5; // 清洁型退役放缓（零燃料成本扛得住）
export const COMP_PEAKER_WAR_SHARE = 0.45; // 玩家市占超此值 → 激进型发动价格战
export const COMP_PEAKER_WAR_FLOOR = 0.72; // 价格战时边际成本最低降至基准的此比例
export const COMP_PEAKER_ADJUST_RATE = 0.25; // 价格战报价向目标靠拢速度（每天）
export const COMP_LEAD_SNATCH_CHANCE = 0.6; // 招商机会过期未签时被最强对手抢走的概率
export const COMP_LEAD_SNATCH_FRACTION = 0.5; // 抢单后对手新增容量 = 客户规模 × 此值
...
export const CAPACITY_CREDIT: Record<PlantType, number> = {
  ...
  hydro: 0.7, biomass: 1, // 水电受枯水期制约、生物质=可靠基荷
};
...
export const AS_HYDRO_REG_FACTOR = 0.4; // 水电可提供调频的容量比例（快速可调）
...
// 分类电价系数：居民小用户付溢价、工业大用户享批发折扣、商业居中；大客户各异。
export const TARIFF_CLASS: Record<LoadProfile, number> = {
  residential: 1.25, commercial: 1.0, industrial: 0.8,
  datacenter: 1.22, // 数据中心愿付溢价换高可靠
  transport: 0.98, // 大交通枢纽
  petrochem: 0.82, // 石化大用户议价
  mining: 0.85, // 矿业大用户议价
};
// 失负荷敏感度：>1 表示该类停电的罚款/口碑代价更高（数据中心 SLA 最敏感）
export const RELIABILITY_WEIGHT: Record<LoadProfile, number> = {
  residential: 1, commercial: 1, industrial: 1,
  datacenter: 2.6, transport: 1.5, petrochem: 1.2, mining: 1.0,
};
// 售电分类报表的三大宏观桶映射（大客户归入最贴近的宏观类）
export const LOAD_MACRO: Record<LoadProfile, 'residential' | 'commercial' | 'industrial'> = {
  residential: 'residential', commercial: 'commercial', industrial: 'industrial',
  datacenter: 'industrial', transport: 'commercial', petrochem: 'industrial', mining: 'industrial',
};

// —— 大客户负荷（招商引资可建造）：付接入投资换大额售电，但各有运营要求 ——
export interface KeyAccountSpec {
  label: string;
  profile: LoadProfile;
  baseDemand: number; // 默认规模 MW
  growthPerHour: number; // 增长率（/仿真小时）
  connectionCapex: number; // 接入投资 ¥
  buildDays: number; // 接入工期（天）
  color: number;
  icon: string;
  desc: string;
}
...
export const CONTRACT_DISCOUNT = 0.08; // 长约期内电价折让（−8%）换取留存
export const KEY_ACCOUNTS: Record<string, KeyAccountSpec> = {
  ...
  petrochem: { label: '石油化工·LNG', profile: 'petrochem', baseDemand: 72, growthPerHour: 0.0025, connectionCapex: 170_000, buildDays: 3, color: 0xef4444, icon: '🛢', desc: '巨型连续基荷·低价·可中断（紧张时削减）' },
  mining: { label: '矿业', profile: 'mining', baseDemand: 60, growthPerHour: 0.003, connectionCapex: 150_000, buildDays: 3, color: 0x9ca3af, icon: '⛏', desc: '巨型平稳·低价·多偏远（长距离输电/电压挑战）' },
};
...
export const BLACKSTART_TYPES: Record<PlantType, boolean> = {
  ...
  hydro: true, // 水电是经典黑启动电源（闸门即开即发）
  nuclear: false, coal: false, wind: false, solar: false, biomass: false, // 需外部电源启动辅机/缺乏调频
};
