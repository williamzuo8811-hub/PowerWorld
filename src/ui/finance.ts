// 财务报表面板：分页签（资产·融资 / 损益 / 市场·对手 / 合约·风控），复用 #panel。
...
export interface FinanceData {
  ...
  competitors: { name: string; capacity: number; marginalCost: number; style: string; acqTotal: number; acqRemedy: number; acqBlocked: boolean; postShare: number }[];
...
const TAB_LABELS = ['💰 资产·融资', '📋 损益', '📈 市场·对手', '📜 合约·风控'];

export class FinancePanel {
  private el = document.getElementById('panel')!;
  private activeTab = 0; // 记住上次打开的页签
  ...
  show(o: FinancePanelOptions): void {
    ...
    panel.innerHTML = `<h1>📊 财务报表</h1><p class="sub">资产负债 · 每日损益（估算）· 市场行情 · 合约风控${d.policy ? ` · <b class="freq-warn">${d.policy}</b>` : ''}</p>`;

    // —— 页签栏 ——
    const tabBar = document.createElement('div');
    tabBar.style.cssText = 'display:flex;gap:6px;margin:4px 0 10px;flex-wrap:wrap';
    const tabBtns: HTMLButtonElement[] = [];
    const tabBodies: HTMLDivElement[] = TAB_LABELS.map(() => document.createElement('div'));
    TAB_LABELS.forEach((lab, i) => {
      const b = document.createElement('button');
      b.textContent = lab;
      b.style.cssText = 'background:#182431;color:var(--text);border:1px solid var(--panel-border);border-radius:8px;padding:7px 13px;cursor:pointer;font-family:inherit;font-size:12px';
      b.onclick = () => {
        this.activeTab = i;
        tabBtns.forEach((x, j) => { x.style.background = j === i ? 'var(--accent)' : '#182431'; x.style.color = j === i ? '#04211a' : 'var(--text)'; });
        tabBodies.forEach((x, j) => { x.style.display = j === i ? 'block' : 'none'; });
      };
      tabBtns.push(b);
    ...
    const mkBtn = (parent: HTMLElement, text: string, enabled: boolean, fn: () => void, title = '') => {
      const b = document.createElement('button');
      b.textContent = text;
      b.disabled = !enabled;
      b.title = title;
    ...
    {
      const t = tabBodies[1];
      t.innerHTML = section('每日损益（按当前运行估算）')
    ...
    // ===== Tab 2：市场·对手 =====
    {
      ...
      t.insertAdjacentHTML('beforeend', section('竞争对手（并购 / 反垄断审查）'));
      const styleLabel: Record<string, string> = { green: '🌱清洁扩张型', coal: '🏭火电基荷型', peaker: '⚔激进调峰型' };
      ...
      d.competitors.forEach((c, i) => {
        ...
        rowEl.innerHTML = `<span style="color:var(--text-dim);font-size:12px">· ${c.name}（${styleLabel[c.style] ?? ''}）· ${c.capacity.toFixed(0)}MW @¥${c.marginalCost.toFixed(0)}/MWh · 并后市占 ${(c.postShare * 100).toFixed(0)}%${remedyNote}</span>`;
      ...
      t.appendChild(compBlock);

      // 批发市场互联
      t.insertAdjacentHTML('beforeend', section(`批发市场（${d.marketEnabled ? '已接入' : '未接入'} · 联络线 ${INTERCONNECTOR_CAPACITY}MW · 购电 ${d.marketImport.toFixed(0)} / 外送 ${d.marketExport.toFixed(0)}MW · 日费 ¥${fmt(MARKET_FEE_PER_DAY)}）`));
      ...
      t.appendChild(mktBtns);

      // 输电权 FTR
      t.insertAdjacentHTML('beforeend', section(`输电权 FTR（持有 ${d.ftrMW.toFixed(0)}MW · 收南北价差 ¥${(d.zoneSouth - d.zoneNorth).toFixed(0)}）`));
      ...
      t.appendChild(ftrBtns);

      // 远期容量
      t.insertAdjacentHTML('beforeend', section(`远期容量（已承诺 ${d.capCommitMW.toFixed(0)}MW · 锁定容量价对冲波动，须交付）`));
    ...
    // ===== Tab 3：合约·风控 =====
    {
      const t = tabBodies[3];
      // 燃料长约
      t.innerHTML = section('燃料长约（锁定燃料价格指数，对冲涨价）');
      ...
      t.appendChild(fuelBtns);

      // 套期保值
      t.insertAdjacentHTML('beforeend', section(`套期保值（远期报价 ¥${d.avgSpot.toFixed(0)}/MWh · 锁价以平抑现货波动）`));
      ...
      t.appendChild(hedgeBtns);

      // 储能商业策略（套利 vs 调频，二选一——同一容量不能两头吃）
      t.insertAdjacentHTML('beforeend', section(`储能商业策略（当前：${d.storageStrategy === 'arb' ? '💹 专注套利' : '📡 投标调频'} · 套利 +¥${fmt(Math.max(0, d.storageArbDay))}/天）`));
      const stratNote = document.createElement('div');
      stratNote.style.cssText = 'font-size:12px;color:var(--text-dim);margin:2px 0 6px';
      stratNote.textContent = d.storageStrategy === 'arb'
        ? '专注价差套利：全额捕获高放低充价差，但不参与调频市场。'
        : '投标调频：储能容量计入调频供给赚取容量费，但需保留 SoC 履约——套利捕获减半。';
      t.appendChild(stratNote);
      const stratBtns = document.createElement('div');
      stratBtns.style.cssText = 'display:flex;gap:6px;margin-bottom:6px';
      mkBtn(stratBtns, d.storageStrategy === 'arb' ? '切换为投标调频' : '切换为专注套利', true, () => o.onToggleStorageStrategy());
      t.appendChild(stratBtns);

      // 设备保险
      t.insertAdjacentHTML('beforeend', section(`设备保险（${d.insured ? '已投保' : '未投保'} · 日保费 ¥${fmt(d.premiumPerDay)} · 赔付 80%）`));
      ...
      t.appendChild(insBtns);

      // 需求响应
      t.insertAdjacentHTML('beforeend', section(`需求响应（${d.demandResponse ? '已启用' : '未启用'} · 高价时削峰 · 当前削减 ${d.drCurtailed.toFixed(0)}MW）`));
      ...
      t.appendChild(interBtns);
    }

    for (const body of tabBodies) panel.appendChild(body);
    // 激活记忆中的页签
    tabBtns[this.activeTab].click();
